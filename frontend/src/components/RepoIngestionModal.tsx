...
import { X, GitBranch, Loader2, Server, Ban, FolderGit2, Link } from "lucide-react";
                ...
                <div className="space-y-4">
                  {!providerToken ? (
                    ...
                    <div className="space-y-2">
                      <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Select a repository to index</p>
                      <div className="grid gap-2 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                        {githubRepos.map(repo => (
                          <button
                            key={repo.id}
                            disabled={isIngesting}
                            onClick={() => executeIngest(repo.clone_url, providerToken, repo.default_branch)}
